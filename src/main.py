...
def _extract_links_from_html(html: str, base_url: str) -> set[str]:
    ...
    return {f"{base_url}{_match.group(1)}" for _match in re.finditer(r'<a ?href="(\/.+)">(.+)<\/a>', html)}
...
def main():
    ...
    while pages_to_visit and debug_counter < float("inf"):
        ...
        if error:
            ...
            _loger.warning(f"Failed to save page {url}. Status code {error.status_code}")
            continue
        ...
        for link in links_on_page:
            ...
            for excluded_path in settings.paths_to_exclude:
                if excluded_path in link:
                    ...
                    break
            if link not in skipped_urls and link not in visited_pages and link not in pages_to_visit:
